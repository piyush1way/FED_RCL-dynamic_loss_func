...
#             # Debugging: Print feature shapes before pooling
#             print(f"Layer {l} local feature shape BEFORE pooling: {local_feature_l.shape}")
#             print(f"Layer {l} global feature shape BEFORE pooling: {global_feature_l.shape}")
    
#             if len(local_feature_l.shape) == 4:  # If shape is (batch, C, H, W)
...
#             batch_size = local_feature_l.size(0)
#             local_feature_l = local_feature_l.view(batch_size, -1)
#             global_feature_l = global_feature_l.view(batch_size, -1)
    
#             # Debugging: Print feature shapes after pooling & flattening
#             print(f"Layer {l} local feature shape AFTER pooling: {local_feature_l.shape}")
#             print(f"Layer {l} global feature shape AFTER pooling: {global_feature_l.shape}")
    
#             # Compute feature cosine similarity loss for alignment
#             if self.args.client.feature_align_loss.align_type == "l2":
#                 loss_cossim = F.mse_loss(local_feature_l, global_feature_l)
#             else:
#                 loss_cossim = F.cosine_embedding_loss(
#                     local_feature_l, global_feature_l, torch.ones_like(labels)
#                 )
...
#                         if train_layer:
#                             # Call ContrastiveLoss
#                             loss_rcl = rcl_criterion(
#                                 z_prev=global_feature_l,  # Global feature as reference
#                                 z_present=local_feature_l,  # Local feature (client model)
#                                 z_serv=global_feature_l,  # Server feature for reg.
#                                 labels=labels,  
...
#                 print(f"Error stacking losses for {loss_name}: {e}")
#                 breakpoint()
