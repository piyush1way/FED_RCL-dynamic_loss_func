...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, z_prev, z_present, z_serv, labels=None, epoch=None):
        ...
        # Flatten the features if they're not already 2D
        if len(z_prev.shape) == 4:  # If it's [batch_size, channels, height, width]
            z_prev = z_prev.view(z_prev.size(0), -1)
            z_present = z_present.view(z_present.size(0), -1)
            z_serv = z_serv.view(z_serv.size(0), -1)
        ...
        # Compute pairwise similarity matrices - compute across batch dimension
